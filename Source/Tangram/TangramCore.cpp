...
typedef int(__stdcall* InitCloudApp)(bool bSupportCrashReporting, void*);
InitCloudApp g_pInitAppFunction = nullptr;
...
CTangram::CTangram()
{
	...
	m_pTangramMessagePumpImpl = nullptr;
	m_pTangramMessagePumpImplProxy = nullptr;
	m_strStartupURL = _T("");
...
{ 
	...
	m_bCLRStart = (BOOL)::GetModuleHandle(_T("mscoreei.dll"));
	...
	if (m_nAppID != 9 && ::PathFileExists(m_strConfigFile) && m_bOfficeApp == false)
	{
		...
		{
			CString strURL = m_Parse.attr(_T("appurl"), _T(""));
			m_bEclipse = m_Parse.attrBool(_T("eclipseapp"));
			if (strURL == _T(""))
			{
				CString strPath = m_strAppPath + _T("startup.html");
				if (::PathFileExists(strPath))
					m_strStartupURL = strPath;
			}
			else
			{
		...
		{
			CString strURL = m_strAppPath + m_strExeName + _T(".htm");
			if (::PathFileExists(strURL) == false && m_bEclipse == false)
			{
				m_strStartupURL = _T("chrome://newtab/");
				m_nAppID = 10000;
			}
		}
	}
...
void CTangram::TangramInit()
{
	...
	if (m_pCreatorImpl&&::IsWindow(m_hChildHostWnd)){
		::PostMessage(m_hHostWnd, WM_TANGRAMMSG, 0, 20190501);
		CString strURL = this->m_strAppPath + m_strExeName + _T(".htm");
		...
		//strURL += _T("|");
		m_hHostBrowserWnd = m_pCreatorImpl->CreateBrowser((HWND)m_hChildHostWnd, m_Parse.xml());
	}
}
	...
	case WM_LBUTTONUP:
		...
		if (lpMsg->hwnd != hPWnd)
		{
			...
			if (it != m_mapBrowserWnd.end())
			{
				...
				if (_hPWnd == nullptr)
				{
					if (it->second->m_pOmniboxViewViews->IsFocused() == false)
						return;
					CHtmlWnd * pWnd = it->second->m_pVisibleWebWnd;
					if (it->second->m_pVisibleWebWnd && lpMsg->hwnd != pWnd->m_hWnd)
					{
...
IWndPage* CTangram::ExtendFrame(HWND hFrame, CString strName, CString strKey)
{
	...
	if (it != m_mapFramePage.end())
	{
		...
		pPage->get_NewVersion(&bNewVersion);
		if (!bNewVersion)
		{
			pNode->put_SaveToConfigFile(true);
		}
...
CString CTangram::InitEclipse(_TCHAR* jarFile)
{
	...
	JNIEnv* pJVMenv = m_pTangramApplicationImpl->m_pJVMenv;
...
void CTangram::CreateEclipseApp(CString strKey, CString strXml)
{
	...
	if (m_bEclipse)
	{
		...
		{
			m_pTangramAppProxy = m_pTangramApplicationImpl->m_pTangramAppProxy;
		}
		if (m_pTangramAppProxy)
		{
...
CWndNode* CTangram::ExtendEx(long hWnd, CString strExXml, CString strXml)
{
	...
	if (pWnd)
	{
		...
		_pFrame->m_bChromeFrame = strName == _T("Chrome Extended Window Class");
	}
...
STDMETHODIMP CTangram::put_AppKeyValue(BSTR bstrKey, VARIANT newVal)
{
	...
	if (strKey.CompareNoCase(_T("TangramApplicationImpl")) == 0 && newVal.llVal)
	{
		...
		CTangramApplicationImpl* pAppImpl = (CTangramApplicationImpl*)newVal.llVal;
		if (pAppImpl->m_pTangramAppProxy)
		{
			pAppImpl->m_pTangramAppProxy->RegistWndClassToTangram();
			bExeModel = (pAppImpl->m_pTangramAppProxy->m_hInstance == ::GetModuleHandle(nullptr));

			CString strID = CString(pAppImpl->m_pTangramAppProxy->m_strProxyID).MakeLower();
			m_mapTangramAppProxy[strID] = pAppImpl->m_pTangramAppProxy;
			pAppImpl->m_pTangramAppProxy->m_pTangramProxyBase = (CTangramProxyBase*)this;
			if (m_pTangramApplicationImpl == nullptr)
				m_pTangramApplicationImpl = pAppImpl;
			if (m_pTangramApplicationImpl&&m_pTangramAppProxy == nullptr)
			{
				m_pTangramAppProxy = pAppImpl->m_pTangramAppProxy;
			}
			if (m_pTangramApplicationImpl&&m_pTangramAppProxy)
			{
				if (m_bEclipse)
				{
					if (launchMode != -1)
				...
				{
					pAppImpl->m_pTangramAppProxy->CreateNewFrame(_T("default"));
					return S_FALSE;
				}
				return S_OK;
			}
	...
	if (strKey.CompareNoCase(_T("CLRProxy")) == 0)
	{
		...
		{
			m_pCLRProxy = (CTangramCLRProxyImpl*)newVal.llVal;
...
{ 
		...
		if (pApp)
		{
			...
			else if (strAppID == _T("chromeplus.tangram"))
			{
				...
				if (pRemoteTangram)
				{
					...
					pRemoteTangram->put_AppKeyValue(CComBSTR(L"ChromeAppData"), CComVariant(bstrXml));
				}
			}
			else if (strAppID == _T("chromeplus")&&::GetModuleHandle(_T("chrome.dll")))
			{
				if (::IsWindow(m_hHostBrowserWnd))
				{
					...
					if (it != g_pTangram->m_mapBrowserWnd.end())
					{
						HWND hBrowser = m_pCreatorImpl->CreateBrowser(NULL, OLE2T(bstrXml));
					}
				}
			}
			else
	...
	else// (it != m_mapRemoteTangramCore.end())
	{
		...
		//it->second->CreateOfficeDocument(NULL);
	}
	return S_OK;
}
			...
			for (auto it1 : it->second->m_mapFrame)
			{
				if (it1.second != it->second->m_pBKFrame)
				{
					...
					if (pWnd&&pWnd->m_bMdiForm)
					{
						...
						CString s = _T("");
						s.Format(_T("<%s>%s</%s>"), it1.second->m_strFrameName, strXml, it1.second->m_strFrameName);
						_strXml += s;
					}
				}
			}
...
ITangramDoc* CTangram::ConnectTangramDoc(CTangramAppProxy* AppProxy, LONGLONG docID, HWND hView, HWND hFrame, LPCTSTR strDocType)
{
...
{
	//if (g_pTangram->m_bTangramInit == false)
	//	g_pTangram->Init();
	return static_cast<ITangram*>(g_pTangram);
}

__declspec(dllexport) CTangramProxyBase* __stdcall  GetTangram2()
{
	return static_cast<CTangramProxyBase*>(g_pTangram);
}
...
__declspec(dllexport) void __stdcall  SetMainDllLoader(ChromePlus::CTangramMainDllLoader* pLoader, void* pDefaultMessagePumpProxy)
{
	g_pTangram->m_pTangramMainDllLoader = pLoader;
	if (pDefaultMessagePumpProxy)
	{
		g_pTangram->m_pTangramMessagePumpImplProxy = (ChromePlus::CTangramMessagePumpImplProxy*)pDefaultMessagePumpProxy;
	}
	::SendMessage(g_pTangram->m_hTangramWnd, WM_TANGRAMMSG, 0, 20190418);
	//::PostAppMessage(g_pTangram->m_dwThreadID, WM_TANGRAMMSG, 0, 20190418);
	//int i = 0;
	//MSG msg;
	//while (GetMessage(&msg, 0, 0, 0) > 0)
	//{
	//	if (msg.message == WM_TANGRAMMSG)
	//	{
	//		if (msg.lParam==20190418)
	//		{
	//			DispatchMessage(&msg);
	//		Sleep(500);
	//		break;
...
__declspec(dllexport) void __stdcall  InitCloudDesktopApp(bool bSupportCrashReporting, void* pAppImpl)
{
	if (g_pTangram)
	{
		CString strChromeCore = g_pTangram->m_strAppPath + _T("tangramchromecore.dll");
		if (::PathFileExists(strChromeCore))
		{
			HMODULE hModule = ::LoadLibrary(strChromeCore);
			if (hModule)
			{
				g_pInitAppFunction = (InitCloudApp)::GetProcAddress(hModule, "InitApp");
				if (g_pInitAppFunction != NULL) {
					g_pInitAppFunction(bSupportCrashReporting, pAppImpl);
				}
			}
		}
	}
...
BOOL CTangram::InitJNIForTangram()
{
	...
	if (m_pTangramApplicationImpl&&m_pTangramApplicationImpl->m_pJVMenv)
		_pJVMenv = m_pTangramApplicationImpl->m_pJVMenv;
	else
	{
...
{
	ChromePlus::CChromeRenderFrameHostBase* pHost = (ChromePlus::CChromeRenderFrameHostBase*)pFrameHostBase;
	...
	if (it != g_pTangram->m_mapHtmlWnd.end())
	{
		...
		if (pVoid)
		{
			it->second->m_pChromeRenderFrameHost->m_pMsg = (ChromePlus::IPCMsg*)pVoid;
			::PostMessage(hHtmlWnd, WM_TANGRAMMSG, 20181220, 0);
		}
		else
		{
			// case 2
...
void CTangram::SetTangramMessagePumpImpl(CTangramMessagePumpImpl* pTangramMessagePumpImpl)
{
	if (g_pTangram->m_pTangramMessagePumpImplProxy && g_pTangram->m_pTangramMessagePumpImpl == nullptr)
	{
		pTangramMessagePumpImpl->m_pProxy = g_pTangram->m_pTangramMessagePumpImplProxy;
		g_pTangram->m_pTangramMessagePumpImpl = pTangramMessagePumpImpl;
	}
}
...
CString CTangram::GetProcessPath(const char* _ver, CString process_type)
{
	if (g_pTangram == nullptr)
		return _T("");
	CString strRet = _T("");
	strRet.Format(_T("%stangram%s.exe"), g_pTangram->m_strAppPath, process_type);
	if (::PathFileExists(strRet))
		return strRet;
	USES_CONVERSION;
	strRet.Format(_T("%s%s\\tangram%s.exe"), g_pTangram->m_strAppPath, A2W(_ver), process_type);
	if (::PathFileExists(strRet))
		return strRet;
	return _T("");
}
...
{
	return g_pTangram->m_strExeName.MakeLower();
}

__declspec(dllexport) ChromePlus::CChromeProcessProxy* __stdcall GetChromeProcessProxy()
{
	return static_cast<ChromePlus::CChromeProcessProxy*>(g_pTangram);
}
