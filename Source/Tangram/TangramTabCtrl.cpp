...
void CTangramListCtrl::OnLvnItemchanged(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	else if (strText.CompareNoCase(_T("new Template"))==0)
	{
		...
		m_pTangramTabCtrl->m_pFrame->Extend(CComBSTR("newtemplate"), CComBSTR(strPath), &pNode);
	}
	else
	{
		...
		m_pTangramTabCtrl->m_pFrame->Extend(CComBSTR("defaultListView"), CComBSTR(strXml), &pNode);
	}
...
void CTangramListCtrl::OnNMDblclk(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if (nItem == -1)
	{
		...
		if (pFrame->m_pWebWnd)
		{
			...
			pFrame->Extend(strKey.AllocSysString(), CComBSTR(g_pTangram->m_strDefaultXml), &pNode);
		}
		else
		{
	...
	case 0:
	{
		...
		else
		{
			...
			{
				hParent = m_pTangramTabCtrl->m_pWndNode->m_pTangramNodeCommonData->m_pFrame->m_pPage->m_hWnd;
				if (hParent)
				{
