...
void MessagePumpForUI::DoRunLoop() {
  ...
  ChromePlus::CTangramMessagePumpImplProxy* pProxy =
  ...
  for (;;) {
    ...
    // work.
    if (pProxy)
      pProxy->OnAppIdle(bIdle, lIdleCount);
    ...
    // begin Add by TangramTeam
    if (pProxy) {
      more_work_is_plausible = pProxy->IsAppIdleMessage();
      if (more_work_is_plausible) {
        bIdle = TRUE;
        lIdleCount = 0;
      }
      more_work_is_plausible = pProxy->DoIdleWork();
    }
    if (more_work_is_plausible)
      continue;
    // end Add by TangramTeam
...
//
//	// begin Add by TangramTeam
//    if (((base::sequence_manager::internal::
//              ThreadControllerWithMessagePumpImpl*)state_->delegate)
//            ->m_pProxy) {
...
bool MessagePumpForUI::ProcessMessageHelper(const MSG& msg) {
  ...
  // begin Add by TangramTeam
  ChromePlus::CTangramMessagePumpImplProxy* pProxy =
      ((base::sequence_manager::internal::ThreadControllerWithMessagePumpImpl*)state_->delegate)->m_pProxy;
  if (pProxy) {
    pProxy->ProcessMsg((MSG*)&msg);
  } else {
    // end Add by TangramTeam
