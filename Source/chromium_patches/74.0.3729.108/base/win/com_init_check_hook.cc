...
// surfacebook2 Function Padding with "mov edi,edi"
const unsigned char g_hotpatch_placeholder_surfacebook2[] = {
    0x00, 0xcc, 0xcc, 0xcc, 0xcc, 0x8b, 0xff};
...
class HookManager {
 ...
 private:
  enum class HotpatchPlaceholderFormat {
    ...
    NOP,
    // begin Add by TangramTeam
    // The hotpatch placeholder used surfacebook2's in the sled.
    SURF2,
    // end Add by TangramTeam
  ...
  void RevertHook() {
    ...
    switch (hotpatch_placeholder_format_) {
      ...
      case HotpatchPlaceholderFormat::NOP:
        ...
        break;
		// begin Add by TangramTeam
      case HotpatchPlaceholderFormat::SURF2:
        if (WasHotpatchChanged())
          return;
        revert_result = internal::ModifyCode(
            reinterpret_cast<void*>(co_create_instance_padded_address_),
            reinterpret_cast<const void*>(&g_hotpatch_placeholder_surfacebook2),
            sizeof(g_hotpatch_placeholder_surfacebook2));
        break;
		// end Add by TangramTeam
  ...
  HotpatchPlaceholderFormat GetHotpatchPlaceholderFormat(const void* address) {
	...
	if (::memcmp(
            reinterpret_cast<void*>(co_create_instance_padded_address_),
            reinterpret_cast<const void*>(&g_hotpatch_placeholder_surfacebook2),
            sizeof(g_hotpatch_placeholder_surfacebook2)) == 0) {
      return HotpatchPlaceholderFormat::SURF2;
    }
    // end Add by TangramTeam
