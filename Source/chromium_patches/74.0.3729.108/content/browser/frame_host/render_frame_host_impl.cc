...
ChromePlus::IPCMsg* g_pMsg = nullptr;
...
RenderFrameHostImpl::RenderFrameHostImpl(SiteInstance* site_instance,
  ...
  // begin Add by TangramTeam
  if (parent_ == nullptr && g_pProxy)
    g_pProxy->m_pCreatingChromeRenderFrameHostBase = this;
  if (g_pMsg) {
    m_pMsg = g_pMsg;
    g_pMsg = nullptr;
  }
  /* if (g_strWorkspaceID.length() != 0) {
       m_strWorkspaceID = g_strWorkspaceID;
       g_strWorkspaceID = L"";
     }
    if (g_strRegionID.length() != 0) {
       m_strRegionID = g_strRegionID;
       g_strRegionID = L"";
     }*/
  // end Add by TangramTeam
}
...
bool RenderFrameHostImpl::OnMessageReceived(const IPC::Message &msg) {
  ...
  IPC_BEGIN_MESSAGE_MAP(RenderFrameHostImpl, msg)
    ...
    // begin Add by TangramTeam
    IPC_MESSAGE_HANDLER(FrameHostMsg_TangramExtend, OnTangramExtend)
    IPC_MESSAGE_HANDLER(FrameHostMsg_ExternalIPCMsg, OnExternalIPCMsg)
    IPC_MESSAGE_HANDLER(FrameMsg_WndNodeIPCMsg, OnWndNodeIPCMsg)
    IPC_MESSAGE_HANDLER(FrameMsg_IPCMsg, OnIPCMessageReceived)
...
void RenderFrameHostImpl::OnDocumentOnLoadCompleted() {
  ...
  if (g_pProxy) {
    ChromePlus::CChromeRenderFrameHostBase* pHost =
        static_cast<ChromePlus::CChromeRenderFrameHostBase*>(this);
    g_pProxy->OnDocumentOnLoadCompleted(pHost, hWnd, pHost->m_pMsg);
  }
  // end Add by TangramTeam
}
...
// begin Add by TangramTeam
void RenderFrameHostImpl::OnTangramExtend(std::wstring xml,
                                          std::wstring key,
  ...
  if (::IsWindow(hWnd)) {
    ChromePlus::IPCMsg IPCInfo;
    IPCInfo.m_strType = xml.c_str();
    IPCInfo.m_strKey = key.c_str();
    IPCInfo.m_strData = features.c_str();
    ::SendMessage(hWnd, WM_CHROMEOPENWINDOWMSG, (WPARAM)this, (LPARAM)&IPCInfo);
  }
}

void RenderFrameHostImpl::OnExternalIPCMsg(std::wstring strType,
                                           std::wstring strKey,
                                           std::wstring strData) {
  HWND hWnd = nullptr;
  RenderWidgetHostViewBase* view = static_cast<RenderWidgetHostViewBase*>(
  ...
  if (::IsWindow(hWnd)) {
    ChromePlus::IPCMsg IPCInfo;
    IPCInfo.m_strType = strType.c_str();
    IPCInfo.m_strKey = strKey.c_str();
    IPCInfo.m_strData = strData.c_str();
    ::SendMessage(hWnd, WM_CHROMEOPENWINDOWMSG, (WPARAM)this, (LPARAM)&IPCInfo);
  }
}

void RenderFrameHostImpl::OnWndNodeIPCMsg(__int64 nHandle,
                                          std::wstring strKey,
                                          std::wstring strData) {
  HWND hWndNode = (HWND)nHandle;
  if (::IsWindow(hWndNode)) {
    ChromePlus::IPCMsg IPCInfo;
    IPCInfo.m_strType = L"";
    IPCInfo.m_strKey = strKey.c_str();
    IPCInfo.m_strData = strData.c_str();
    ::SendMessage(hWndNode, WM_CHROMEWNDNODEMSG, (WPARAM)0, (LPARAM)&IPCInfo);
  }
}

void RenderFrameHostImpl::OnSetFocusedFrame() {
  SetFocusedFrame();
}

std::string RenderFrameHostImpl::Get_LastCommittedURL() {
  std::string strRet = GetLastCommittedURL().spec();
  CStringA str = CStringA(strRet.c_str());
  return "";
}

void RenderFrameHostImpl::OpenURLWithID(std::wstring strUrl,
                                        BrowserWndOpenDisposition nPos,
                                        std::wstring strWorkspaceID,
                                        std::wstring strRegionID) {
  WebContents* pContents = delegate_->GetAsWebContents();
  if (pContents) {
    /*   g_strWorkspaceID = strWorkspaceID;
     g_strRegionID = strRegionID;*/
    pContents->OpenURL(content::OpenURLParams(
        GURL(strUrl), content::Referrer(), (WindowOpenDisposition)nPos,
        ui::PAGE_TRANSITION_LINK, false /* is_renderer_initiated */));
  }
}

void RenderFrameHostImpl::InternalSend(ChromePlus::IPCMsg* pMsg) {
  if (pMsg->m_strType.CompareNoCase(_T("NODE_MESSAGE:OPENURL")) == 0) {
    WebContents* pContents = delegate_->GetAsWebContents();
    if (pContents) {
      int nPos = pMsg->m_strKey.Find(_T("|"));
      CString strKey = pMsg->m_strKey.Left(nPos);
      CString _strKey = pMsg->m_strKey.Mid(nPos + 1);
      nPos = _strKey.Find(_T("|"));
      g_pMsg = pMsg;
      pContents->OpenURL(content::OpenURLParams(
          GURL(LPCTSTR(_strKey.Mid(nPos + 1))), content::Referrer(),
          (WindowOpenDisposition)_wtoi(_strKey.Left(nPos)),
          ui::PAGE_TRANSITION_LINK, false /* is_renderer_initiated */));
    }
    return;
  } else
    Send(new FrameMsg_InternalIPCMsg(routing_id_, LPCTSTR(pMsg->m_strType),
                                     LPCTSTR(pMsg->m_strKey),
                                     LPCTSTR(pMsg->m_strData)));
...
void RenderFrameHostImpl::OnIPCMessageReceived(
    std::wstring strContent,
    std::vector<std::string> arguments) {
  for (auto it = arguments.begin(); it != arguments.end(); it++) {
    std::string argument = (std::string)*it;
    ChromePlus::TUnit* u;
    ChromePlus::TUnit::NewFromBuffer(argument.data(), &u);
    switch (u->type) {
      case 1: {
        int32_t result = u->ToInt32();
        ATLTRACE(_T("IPCMessage: ************ %d **************"), result);
        break;
      }
      case 4: {
        wchar_t* wszString;
        u->ToWCharArray(&wszString);
        // char* result;
        // u->ToCharArray(&result);
        // int wcsLen = ::MultiByteToWideChar(CP_UTF8, NULL, result,
        // strlen(result), NULL, 0);  wchar_t* wszString = new
        // wchar_t[wcsLen + 1];
        //::MultiByteToWideChar(CP_UTF8, NULL, result, strlen(result),
        // wszString, wcsLen);  wszString[wcsLen] = '\0';
        ATLTRACE(_T("IPCMessage: ************ %s **************\n"), wszString);
        // delete result;
        delete[] wszString;
        break;
      }
      case 5: {
        double result = u->ToDouble();
        time_t t = static_cast<time_t>(result / 1000);
        tm* ptm = localtime(&t);
        char buffer[32];
...
void RenderFrameHostImpl::OnTangramMessage(std::wstring channel,
  ...
  if (::IsWindow(hWnd)) {
    ...
    ::SendMessage(hWnd, WM_CHROMEIPCMSG, (WPARAM)&IPCInfo, 0);
  }
}

void RenderFrameHostImpl::SendTangramMessage(std::wstring channel,
                                             std::wstring arg1,
                                             std::wstring arg2) {
  // if (channel == L"NODE_MESSAGE:OPENURL") {
  //  WebContents* pContents = delegate_->GetAsWebContents();
  //  if (pContents) {
  //    CString m_strKey = arg1.c_str();
  //    int nPos = m_strKey.Find(_T("|"));
  //    CString strKey = m_strKey.Left(nPos);
  //    CString _strKey = m_strKey.Mid(nPos + 1);
  //    nPos = _strKey.Find(_T("|"));
  //    g_pMsg = new ChromePlus::IPCMsg();
  //    g_pMsg->m_strType = channel.c_str();
  //    g_pMsg->m_strKey = arg1.c_str();
  //    g_pMsg->m_strData = arg2.c_str();
  //    pContents->OpenURL(content::OpenURLParams(
  //        GURL(LPCTSTR(_strKey.Mid(nPos + 1))), content::Referrer(),
  //        (WindowOpenDisposition)_wtoi(_strKey.Left(nPos)),
  //        ui::PAGE_TRANSITION_LINK, false /* is_renderer_initiated */));
  ...
  //  return;
  //}
  Send(new TangramFrameMsg_Message(routing_id_, channel, arg1, arg2));
}
// end Add by TangramTeam
