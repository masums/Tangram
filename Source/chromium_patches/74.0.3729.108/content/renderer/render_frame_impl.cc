...
#endif
    // begin Add by TangramTeam
    IPC_MESSAGE_HANDLER(FrameMsg_InternalIPCMsg, OnInternalIPCMsg)
...
// begin Add by TangramTeam
void RenderFrameImpl::OnTangramExtend(std::wstring strXml,
                                      std::wstring strKey,
                                      std::wstring strFeatures) {
  Send(
      new FrameHostMsg_TangramExtend(routing_id_, strXml, strKey, strFeatures));
}

void RenderFrameImpl::ExternalSend(std::wstring strType,
                                   std::wstring strKey,
                                   std::wstring strData) {
  Send(new FrameHostMsg_ExternalIPCMsg(routing_id_, strType, strKey, strData));
}

void RenderFrameImpl::SendNodeMessage(__int64 nHandle,
                                      std::wstring strXml,
                                      std::wstring strData) {
  Send(new FrameMsg_WndNodeIPCMsg(routing_id_, nHandle, strXml, strData));
}

void RenderFrameImpl::OnInternalIPCMsg(std::wstring strType,
                                       std::wstring strKey,
                                       std::wstring strData) {
  blink::WebLocalFrameImpl* web_local_frame_impl_ =
      (blink::WebLocalFrameImpl*)GetWebFrame();
  if (web_local_frame_impl_) {
    blink::Tangram* pTangram =
        web_local_frame_impl_->GetFrame()->DomWindow()->tangram();
    if (strType == L"WndNode_Created") {
      pTangram->m_mapNode[strKey] = _wtoi64(strData.c_str());
    } else if (strType == L"GetElementByID") {
      pTangram->extend("domparse", "GetElementByID", String(strType.c_str()),
                       "");
      return;
    }
    pTangram->DispatchEvent(*blink::TangramEvent::Create(
        blink::event_type_names::kTangrammsg, String(strType.c_str()),
        String(strKey.c_str()), String(strData.c_str())));
  }
}

void RenderFrameImpl::sendMessage(std::wstring strContent,
                                  void* pvoid,
                                  void* pException) {
  base::Optional<Vector<blink::ScriptValue>>* params =
      (base::Optional<Vector<blink::ScriptValue>>*)pvoid;
  if (params) {
    blink::ExceptionState* pExceptionState = (blink::ExceptionState*)pException;
    Vector<blink::ScriptValue>& ScriptValues = params->value();
    std::vector<std::string> arguments;
    for (const blink::ScriptValue& argument : ScriptValues) {
      if (!argument.IsNull()) {
        v8::TryCatch block(argument.GetIsolate());
        String s = "";
        argument.ToString(s);
        v8::Local<v8::Value> value = argument.V8Value();
        if (value->IsNumber()) {
          int32_t result;
          if (!value->Int32Value(argument.GetIsolate()->GetCurrentContext())
                   .To(&result)) {
            pExceptionState->RethrowV8Exception(block.Exception());
          }
          ChromePlus::TUnit unit(result);
          arguments.push_back(unit.ToString());

        } else if (value->IsString()) {
          v8::Local<v8::String> result = v8::Local<v8::String>::Cast(value);
          uint16_t buffer[result->Length() + 1];
          if (result->Write(argument.GetIsolate(), buffer)) {
            ChromePlus::TUnit unit((wchar_t*)buffer);
            arguments.push_back(unit.ToString());
          }
        } else if (value->IsDate()) {
          double result = 0.0f;
          if (v8::Local<v8::Date>::Cast(value)
                  ->NumberValue(argument.GetIsolate()->GetCurrentContext())
                  .To(&result)) {
            ChromePlus::TUnit unit(result);
            arguments.push_back(unit.ToString());
          }
    ...
    Send(new FrameMsg_IPCMsg(routing_id_, strContent, arguments));
  }
}
