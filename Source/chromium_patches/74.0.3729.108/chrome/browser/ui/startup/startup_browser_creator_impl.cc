...
// begin Add by TangramTeam
ChromeBrowserCreatorImpl::ChromeBrowserCreatorImpl() {
  profile_ = nullptr;
  HMODULE hModule = ::GetModuleHandle(L"tangramcore.dll");
  if (hModule) {
    typedef ChromePlus::CTangramProxyBase*(__stdcall * GetTangram2)();
    GetTangram2 _pTangramFunction;
    _pTangramFunction = (GetTangram2)GetProcAddress(hModule, "GetTangram2");
    if (_pTangramFunction != NULL) {
      ChromePlus::g_pTangramBase = _pTangramFunction();
    }
  }
}
...
HWND ChromeBrowserCreatorImpl::CreateBrowser(HWND hParent, CString strXml) {
  HWND hBrowser = NULL;
  if (profile_) {
    StartupTabs tabs;
    bool b = false;
    ...
    if (b == true) {
      if (g_pProxy && ::IsWindow(hParent))
        g_pProxy->m_hParent = hParent;
      ...
      if (::IsWindow(hBrowser) &&
          (::GetWindowLongPtr(hBrowser, GWL_STYLE) & WS_CHILD)) {
      }
      g_pProxy->m_hParent = NULL;
      g_pProxy->OnSubBrowserWndCreated(hParent, hBrowser);
      browser->window()->Show();
    }
  }
  return hBrowser;
...
StartupBrowserCreatorImpl::StartupBrowserCreatorImpl(
    ...
    : cur_dir_(cur_dir),
      ...
      is_first_run_(is_first_run == chrome::startup::IS_FIRST_RUN) {
  // begin Add by TangramTeam
  if (g_pProxy && g_pProxy->m_pCreatorImpl == nullptr) {
    g_pProxy->m_pCreatorImpl = new ChromeBrowserCreatorImpl();
  }
  // end Add by TangramTeam
}
...
bool StartupBrowserCreatorImpl::Launch(Profile* profile,
  ...
  profile_ = profile;
  // begin Add by TangramTeam
  if (g_pProxy && g_pProxy->m_pCreatorImpl) {
    ChromeBrowserCreatorImpl* pImpl =
        (ChromeBrowserCreatorImpl*)g_pProxy->m_pCreatorImpl;
...
Browser* StartupBrowserCreatorImpl::RestoreOrCreateBrowser(
    ...
    bool is_post_crash_launch) {
  // begin Add by TangramTeam
  bool process_crash_launch = false;
  char* url = nullptr;
  if (process_startup) {
    if (g_pProxy) {
      if (ChromePlus::g_pTangramBase->m_bEclipse == false) {
		  USES_CONVERSION;
		  url = W2A(ChromePlus::g_pTangramBase->m_strStartupURL);
      } else if (ChromePlus::g_pTangramBase->m_strStartupURL == _T(""))
        url = nullptr;
    }
    if (url == nullptr) {
      process_crash_launch = true;
      return nullptr;
    }
  }
  // end Add by TangramTeam
