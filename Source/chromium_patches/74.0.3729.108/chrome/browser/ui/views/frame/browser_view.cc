...
void PaintDetachedBookmarkBar(gfx::Canvas* canvas, BookmarkBarView* view) {
...
void PaintBackgroundAttachedMode(gfx::Canvas* canvas,
  ...
  if (theme_provider->HasCustomImage(IDR_THEME_TOOLBAR)) {
    canvas->TileImageInt(*theme_provider->GetImageSkiaNamed(IDR_THEME_TOOLBAR),
                         background_origin.x(), background_origin.y(),
  ...
  if (should_animate) {
    if (!loading_animation_timer_.IsRunning()) {
      ...
      loading_animation_timer_.Start(
          FROM_HERE, TimeDelta::FromMilliseconds(kLoadingAnimationFrameTimeMs),
          this, &BrowserView::LoadingAnimationCallback);
    }
  } else {
    if (loading_animation_timer_.IsRunning()) {
      loading_animation_timer_.Stop();
...
void BrowserView::InitViews() {
  ...
  // HWND hwnd = views::HWNDForWidget(GetWidget());
  // bool bChild =
  //    ::IsWindow(hwnd) && (::GetWindowLongPtr(hwnd, GWL_STYLE) & WS_CHILD);
  ...
  browser_view_layout->Init(new BrowserViewLayoutDelegateImpl(this), browser(),
                            this, top_container_, tabstrip_, toolbar_,
