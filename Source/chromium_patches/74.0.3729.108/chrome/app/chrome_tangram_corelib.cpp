...
int InitApp(bool bSupportCrashReporting, void* pDefaultMessagePumpProxy) {
  ...
  typedef ChromePlus::CTangramMainDllLoader*(__stdcall * SetMainDllLoader)(
      ChromePlus::CTangramMainDllLoader * pLoader, void*);

  HMODULE hModule = ::GetModuleHandle(L"tangramcore.dll");
  if (hModule == nullptr)
	hModule = ::LoadLibrary(L"tangramcore.dll");
  if (hModule == nullptr) {
  ...
  if (pSetMainDllLoader != NULL) {
    pSetMainDllLoader((ChromePlus::CTangramMainDllLoader*)MakeMainDllLoader(),
                      pDefaultMessagePumpProxy);
  }
  return 0;
}

int InitChromeApp(bool bSupportCrashReporting, void* pDefaultMessagePumpProxy) {
  return InitApp(bSupportCrashReporting, pDefaultMessagePumpProxy);
}